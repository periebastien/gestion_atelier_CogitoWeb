...
( function () {
	...
	function onScroll() {
		...
		if ( mq.matches ) {
			...
			// Quitter le haut de page efface le fond du conteneur, dans les DEUX
			// sens : sinon la barre intérieure se translate hors champ mais son
			// conteneur reste peint en blanc en haut de l'écran (bande résiduelle).
			hdr.classList.toggle( 'ar-mob-past', past );
		} else {
			if ( fp ) {
	...
	function reset() {
		...
		hdr.classList.remove( 'ar-mob-past' );
		if ( fp ) {
