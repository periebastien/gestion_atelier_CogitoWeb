<?php
/**
 * Altitude Révision — panier & commande (porté d'AEROTECH le 07/08/2026).
 *
 * Sur ce site seul le CHECKOUT est habillé (woocommerce/checkout/) : la page
 * panier n'est pas dans le tunnel (on arrive du formulaire de demande
 * d'intervention). Ce fichier porte les helpers partagés (icônes du sprite,
 * frise d'étapes) et les endpoints AJAX — dont at_cart_coupon, utilisé par
 * l'accordéon « code promo » du checkout.
 */
...
function at_steps( $current = 1 ) {
	$steps = array( 'Demande d\'intervention', 'Livraison & paiement', 'Confirmation' );
...
function at_stock_label( $product ) {
	...
	if ( $qty !== null && $qty > 0 ) { return 'En stock'; }
...
/* Le filtre AEROTECH de ventes croisées (meilleures ventes injectées dans le
   panier) n'est PAS repris : la boutique de ce site est désactivée. */
